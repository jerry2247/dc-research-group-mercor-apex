...
Detailed results for the **Dynamic Ledger** subsystem evaluated on Mercor's **APEX-v1-extended** benchmark. Numbers are headline-only here; the source of truth is the per-task CSV (`runs/<run>/results.csv`) and the per-task ledger snapshots (`runs/<run>/dynamic_ledger/<Domain>/`).
...
Each cell will accumulate as the rollout proceeds. The per-domain Dynamic Ledger is isolated by design — a separate `dynamic_ledger/<Domain>/` snapshot tree per domain — so the Finance ledger does not carry into Legal/Consulting/Medicine.
