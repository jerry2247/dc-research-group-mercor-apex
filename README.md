...
`--memory` (the baseline pipeline is byte-identical when off).
...
│   │                                                                 │       │
│   │   Inputs:                                                       │       │
│   │     · active playbook (memory subsystem for this domain)          │       │
...
│   └────────────────────────────────┬────────────────────────────────┘       │
│                                    ▼                                        │
│           runs/<run>/memory/<Domain>/snapshot_NNNN.json             │
└─────────────────────────────────────────────────────────────────────────────┘
```

**The five-stage shape and the `<memory_updates>` tag follow Suzgun et
al.'s published memory subsystem spec.** Our extensions, documented in
...
per (task, model). The memory subsystem-on column is intentionally empty
...
| memory subsystem · grok-4.3-high · `--memory` | _pending_ | _pending_ | _pending_ | _pending_ | _pending_ |

### Enabling the memory subsystem
...
log are written to `runs/<run>/memory/<Domain>/`.
...
0. [memory subsystem — our extension](#memory--our-extension)  ←  architecture diagram + results table
