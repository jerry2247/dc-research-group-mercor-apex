...
from apex_bench.trace.embeddings import EmbeddingClient
from apex_bench.trace.reflector import ReflectorProposal
...
def curate(
    ...
    *,
    ...
    """The TRACE curator LLM call. Sees the GT bit AND reflector
    proposals — per the TRACE paper. Returns the final op list."""
    import litellm

    if cfg.curator_model is None:
        raise ValueError(
            "TraceConfig.curator_model is None — the runner must fill it "
            "from the active TestModelProfile before calling curate(). See "
            "docs/TRACE_PRD.md."
        )
