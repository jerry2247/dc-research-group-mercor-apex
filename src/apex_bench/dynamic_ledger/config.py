...
See ``docs/DYNAMIC_LEDGER_PRD.md``. Mirrors the apex-agents-bench
config minus the agentic trajectory-rendering knob.
"""
...
class DynamicLedgerConfig:
    ...
    The curator runs on the SAME model as the agent under test (and uses
    the same thinking effort): if the agent is ``grok-4.3-high`` the
    curator is also ``grok-4.3-high``. Only the judge model is fixed
    (gpt-5.5 medium). Leave ``curator_model`` and
    ``curator_extra_args`` ``None``; the runner fills them in from the
    active :class:`TestModelProfile` before the first curator call.

    Setting them explicitly is allowed for curator-ablation experiments
    that hold the curator model fixed across agent profiles. The CLI
    does not surface those knobs.
    """
    ...
    # The curator model is the agent profile's model_id by default. Set
    # explicitly only for curator-ablation experiments.
