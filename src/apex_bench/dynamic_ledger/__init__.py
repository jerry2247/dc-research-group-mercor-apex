"""Dynamic Ledger — no-ground-truth memory subsystem for apex-bench.

Per ``docs/DYNAMIC_LEDGER_PRD.md``. The Dynamic Ledger is gated by
``DynamicLedgerConfig.enabled``; when off, importing this package has
no effect on the baseline pipeline and the CSV schema is byte-identical
to the no-ledger path.
...
from apex_bench.dynamic_ledger.embeddings import (
    EmbeddingClient,
    LiteLLMEmbeddingClient,
    cosine_similarity,
)
from apex_bench.dynamic_ledger.entry import DynamicLedger, Entry
from apex_bench.dynamic_ledger.injector import (
    augment_user_prompt,
