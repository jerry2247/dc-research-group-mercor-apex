...
``(dynamic_ledger, task_prompt, response, *, cfg)``. **NO**
...
from apex_bench.dynamic_ledger.embeddings import EmbeddingClient
...
set, so the curator can still emit the literal NO_OP string described
in the prompt without poisoning the ledger."""
    ...
    for raw in data:
        ...
        try:
            if op == "CREATE":
                ...
                out.append(
                    CuratedOp(op=op, entry_id=str(raw["entry_id"]), content=str(raw["content"]))
                )
            elif op == "DELETE":
...
def _build_user_message(
    *,
    ...
    return (
        ...
        .replace("{stripped_response}", response)
    )
...
def curate(
    ...
    *,
    ...
    Positional inputs:
      - dynamic_ledger: the per-domain Dynamic Ledger (active entries
        are rendered into the curator's user message as the playbook)
      - task_prompt: verbatim task prompt the colleague saw WITHOUT
        the strategies-block injection
      - response: the deliverable prose, as the colleague submitted it

    Keyword-only:
      - cfg: DynamicLedgerConfig — ``cfg.curator_model`` must be set
        before calling (the runner fills it from the active
        :class:`TestModelProfile` so the curator and the generator
        share the same model and thinking effort)
    ...
    if cfg.curator_model is None:
        raise ValueError(
            ...
            "fill it from the active TestModelProfile before calling "
            "curate(). See docs/DYNAMIC_LEDGER_PRD.md."
        )
