...
When all four domains are complete on baseline + memory subsystem, run TRACE against a fresh CSV at `runs/grok43high-trace/results.csv` (its own per-domain cheatsheet starts empty for each domain).
