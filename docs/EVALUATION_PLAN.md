...
3. **Per domain, the three methods** are run in this order: **baseline (no memory) → memory subsystem → TRACE**. All three share the same task slice, the same test profile, and the same judge; relative order is fixed only for reproducibility.
4. **Resume**: each method has a single CSV per repo (one for baseline, one for memory subsystem, one for TRACE) and the runner appends to it. The memory subsystem and TRACE ledgers carry forward across domains within the same method-CSV through their respective per-domain snapshot directories (`runs/<run>/memory/<Domain>/`, `runs/<run>/trace/<Domain>/`).

The Finance subset was run first as the project pilot; the rest of the rollout follows the rule above.

## Status

| Domain (lex order) | n | baseline | memory subsystem | TRACE |
|---|---:|:---:|:---:|:---:|
| Consulting | 25 | — | — | — |
| Finance | 25 | ✓ | ✓ | — |
| Legal | 25 | — | — | — |
...
When all four domains are complete on baseline + memory subsystem, add `--trace` (against a separate CSV, e.g. `runs/all-grok43high-trace/results.csv`) and run the same domain order from scratch; a new per-domain ledger begins from empty for each domain.
