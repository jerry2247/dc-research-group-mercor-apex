...
emits ops on a task whose agent ultimately failed (the curator still
runs on the failed trajectory, the snapshot is written, but the
runner records no CSV row). Loading the latest snapshot ensures
those entries are never silently dropped on resume.
